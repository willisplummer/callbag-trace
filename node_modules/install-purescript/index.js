...
const createGunzip = zlib.createGunzip;
...
module.exports = function installPurescript(dir, options) {
  return new Observable(observer => {
    ...
    const binName = typeof options.rename === 'function' ? String(options.rename(defaultBinName)) : defaultBinName;
    ...
    function main() {
      subscriptions.add(downloadOrBuildPurescript(dir, options).subscribe({
        ...
        complete(path) {
          subscriptions.add(fileToTar(path, cachePath, Object.assign({
            map(header) {
    ...
    Promise.all([readdirClean(cacheDir), prepareWrite(binPath)]).then(results => {
      if (observer.closed) {
        return;
      }

      const cacheCount = results[0].length;
...
${truncatedList(results[0], MAX_LIST_NUM)}

You can free disk usage by deleting obsolete caches or simply removing the cache directory itself.`;
      }

      subscriptions.add(tarToFile(cachePath, binPath, Object.assign({}, options, {
        map(header) {
