...
const dim = chalk.dim;
const strikethrough = chalk.strikethrough;
...
const render = isPrettyMode ? () => {
  ...
  for (const task of taskGroups[0].values()) {
    const allowFailure = task.allowFailure;
    const byteFormatter = task.byteFormatter;
    const duration = task.duration;
    const head = task.head;
    const status = task.status;
    const subhead = task.subhead;

    if (status === 'done' || status === 'failed') {
      const durationStr = process.stdout.columns > head.length && task.duration >= 100 ? dim.gray(` (${ms(duration)})`) : '';
    ...
    if (task.message) {
      if (status === 'failed') {
        lines.push(`${red(`  ${task.message.replace(/^[ \t]+/, '')}`)}`);
      } else {
  ...
  for (const pairs of taskGroups[0]) {
    const task = pairs[1];
    ...
    const durationStr = task.duration < 100 ? '' : ` (${ms(task.duration)})`;
...
const initialize = once(firstEvent => {
  if (firstEvent.id === 'search-cache') {
    if (firstEvent.found) {
      console.log(`${info}Found a cache at ${
        magenta(tildePath(firstEvent.path))
      }\n`);
    }

    if (firstEvent.warning) {
      console.log(`${yellow(neatFrame(firstEvent.warning))}\n`);
    }
  }
...
function showError(erroredTask, err) {
  ...
  if (err.code === 'ERR_UNSUPPORTED_PLATFORM') {
    ...
    if (argv.platform !== undefined && argv.platform !== process.platform) {
      erroredTask.message = neatFrame(firstLine);
    } else {
      erroredTask.message = neatFrame(`${firstLine}
...
2. Help the community to fix the issue, for exmaple, discuss how to provide prebuilt binaries for ${platform} continuously.`);
    ...
    erroredTask.message = neatFrame(err.message);
  } else if (err.INSTALL_URL) {
    erroredTask.message = neatFrame('\'stack\' command is required for building PureScript from source, ' +
